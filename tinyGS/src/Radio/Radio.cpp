...
// ConfigManager::getInstance().getBoardConfig().L_SX127X=0;
Radio::Radio()
    : spi(SPI)
{
}
...
void Radio::init()
{
  ...
  if (strlen(ConfigManager::getInstance().getBoardTemplate()) > 0)
  {
    ...
    // board.OLED__address =60;
    // board.OLED__SDA = 17;
    // board.OLED__SCL = 18;
    // board.OLED__RST = 21;
    // board.PROG__BUTTON = 0;
    // board.BOARD_LED =35;
    // board.L_SX127X = 0;
    // board.L_NSS = 8;
    // board.L_DI00 = 14;
    ...
    // board.L_MOSI = 10;
    // board.L_SCK = 9;
    // board.L_TCXO_V = 32000000.0;
    // Log::console(PSTR("pin init ok!!!"));
    // Log::console(PSTR("L_SX127X=%d"),board.L_SX127X);
  }
  else
  {
  ...
  if (board.L_SX127X)
  {
        Log::console(PSTR("lora sx1276"));

    lora = new SX1278(new Module(board.L_NSS, board.L_DI00, board.L_DI01, spi, SPISettings(2000000, MSBFIRST, SPI_MODE0)));
  }
  else
  {
    Log::console(PSTR("lora sx1262"));
    lora = new SX1268(new Module(board.L_NSS,board.L_DI00,board.L_RST, board.L_BUSSY, spi, SPISettings(2000000, MSBFIRST, SPI_MODE0)));
  }
...
int16_t Radio::begin()
{
  ...
  board_type board = ConfigManager::getInstance().getBoardConfig();
    // board_type board;
          //  Log::console(PSTR("ConfigManager::getInstance().getBoardConfig().L_SX12xx=:%d"),ConfigManager::getInstance().getBoardConfig().L_SX127X);
...
Log::console(PSTR("sx1278"));
    ...
    else
    {
      state = ((SX1268 *)lora)->begin(m.frequency + status.modeminfo.freqOffset, m.bw, m.sf, m.cr, m.sw, m.power, m.preambleLength,board.L_TCXO_V );
       Log::console(PSTR("sx1262:%d"),board.L_SX127X);
      if (m.fldro == 2)
...
void Radio::startRx()
{
  ...
  if (ConfigManager::getInstance().getBoardConfig().L_SX127X){
    ((SX1278 *)lora)->startReceive();
    Log::console(PSTR("215556"));
  }
    
  else{
 ((SX1268 *)lora)->startReceive();
     Log::console(PSTR("215557"));
